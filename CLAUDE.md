...
Phase 0 (scaffold), Phase 1 (Spotify OAuth & token storage), Phase 2 (data layer & config), Phase 3 (public search & request form), Phase 4 (public menu), and Phase 5 (admin approval workflow, SPEC.md §11) are complete. Phases 6–8 (background worker, hardening, polish) are not yet implemented — `app/worker/` exists as an empty package stub per SPEC.md §13.
...
- Admin requests queue (`/admin/requests`) shows two sections — pending (with approve/deny buttons) and added (read-only) — but does **not** join against `playlist_state.played_at`/`removed_at` to show "has this track played yet," since those columns are never populated by anything until Phase 6's worker exists; that join would always just say "no."
