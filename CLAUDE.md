...
Phase 0 (scaffold), Phase 1 (Spotify OAuth & token storage), and Phase 2 (data layer & config, SPEC.md §11) are complete. Phases 3–8 (public form, public menu, admin approval workflow, background worker, hardening, polish) are not yet implemented — `app/services/` (beyond `crypto.py`) and `app/worker/` exist as empty package stubs per SPEC.md §13.
...
- Default admin user is seeded at startup (`app/main.py:_seed_default_admin_if_needed`) only if `admin_users` is empty, with a random one-time password logged via `logger.warning` (visible in container/process logs, never written to a file or committed anywhere) — not a fixed default credential, since this is a public repo.
- Session cookies use Starlette's `SessionMiddleware` (signed, not server-side-stored) — `same_site="strict"` is hardcoded per SPEC.md §8, but `https_only` is driven by the new `SESSION_COOKIE_SECURE` env var (default `false`) for the same reason `Caddyfile`'s `SITE_ADDRESS` is configurable: a `Secure` cookie is silently dropped by browsers over plain HTTP, so this can't default to `true` until Hostinger's TLS behavior (§12.1) is confirmed.
