...
Still open before Phase 0 fully closes per SPEC.md: push this to a **public** GitHub repo, replace the `<owner>/<repo>` placeholder in `docker-compose.yml`'s git-context build, enable GitHub secret scanning/push protection, set up branch protection requiring CI to pass, and verify Hostinger's actual TLS-termination behavior (SPEC.md §9/§12.1 — the `Caddyfile`'s `SITE_ADDRESS` env var is the toggle once that's known).
